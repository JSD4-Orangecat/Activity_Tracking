...
export default function DashboardAchievements() {
  ...
  const calcAchievementImage = useCallback(
    (activity) => {
      if (activity >= 10) {
        return "Gold";
      } else if (activity >= 5) {
  ...
  if (activitiesType) {
    achievement = [
      {
        count: activitiesType.biking,
        achievementImg: `/medals/Biking/medalBiking${calcAchievementImage(
          activitiesType.biking
        )}.png`,
      },
      {
        count: activitiesType.running,
        achievementImg: `/medals/Running/medalRunning${calcAchievementImage(
          activitiesType.running
        )}.png`,
      },
      {
        count: activitiesType.swimming,
        achievementImg: `/medals/Swimming/medalSwimming${calcAchievementImage(
          activitiesType.swimming
        )}.png`,
      },
      {
        count: activitiesType.cardio,
        achievementImg: `/medals/Cardio/medalCardio${calcAchievementImage(
          activitiesType.cardio
        )}.png`,
      },
      {
        count: activitiesType.walking,
        achievementImg: `/medals/Walking/medalWalking${calcAchievementImage(
          activitiesType.walking
        )}.png`,
      },
    ];
  }
  ...
  const fetchCountActivity = async () => {
    ...
    try {
      const response = await axios.get(
        `${backend}/activities/count/${currentUser._id}`
      );
      const data = {
  ...
  return (
    <div className="achievements-container">
      ...
      <div className="achievements-wrapper">
        ...
        {!achievementLoading &&
          achievement?.map(({ achievementImg, count }, index) => (
            <div key={index}>
